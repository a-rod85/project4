...
def reservation(request):
    ...
    if request.method == 'POST':
        ...
        try:
            order_exists = Reservation.objects.get(Booking_time=str(get_date)
            +"T"+ str(get_time), Restaurant__name = get_Restaurant_name)
            return redirect('Booking_Failed')
        except Reservation.DoesNotExist:
            get_restaurant = Restaurant.objects.get(name=get_Restaurant_name)
            create_booking = Reservation.objects.create(Name=username, 
            Restaurant = get_restaurant, Booking_time=get_date + "T"+get_time)
            create_booking.save()
...
def login_request(request):
	...
	form = NewUserForm()
	return render (request=request, template_name="main/register.html", context={"register_form":form})uest, template_name="main/register.html", context={"register_form":form})
